        ...
        for r in range(2):
            for c in range(3):
                ...
                if c > 0:  # Not in the first column
                    ...
                    ax_main.tick_params(axis='both', which='major', labelsize=GLOBAL_FONT_SIZE)
...
# Tick Labels (X and Y)
                ...
                ax_hist_x.set_title(f"{class_name}, {corr_str}", fontsize=GLOBAL_FONT_SIZE, pad=10)
                correlations.update({class_name: corr_coef})

        # Adjust layout to prevent labels from overlapping
        plt.suptitle(
            "Divergence vs. Signal by Repeat Class (Shared Axes)", fontsize=GLOBAL_FONT_SIZE, y=0.98
        )
        figure.tight_layout(
            rect=[0, 0.03, 1, 0.95]
...
correlation_df.to_csv(f"Signal_vs_divergence_correlation_by_lines_mods_classes_spearman_cut_{str(DIV_THRESHOLD)}.csv")
